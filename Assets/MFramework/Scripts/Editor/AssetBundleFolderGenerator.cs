...
public class AssetBundleFolderGenerator : EditorWindow
{
    ...
    [MenuItem("Tools/AssetBundle/生成双目录文件夹结构")]
    public static void ShowWindow()
    {
        var window = GetWindow<AssetBundleFolderGenerator>("AB包双目录生成器");
    ...
    private readonly string[] tabNames = { "美术资源目录", "AB包目录" };
    ...
    private void OnGUI()
    {
        ...
        // 标题
        EditorGUILayout.LabelField("AssetBundle 双目录结构生成器", EditorStyles.boldLabel);
        EditorGUILayout.Space();

        // 目录说明
        EditorGUILayout.HelpBox(
            "美术资源目录 (Assets/Art): 存放原始美术资源，用于编辑和制作\n" +
            "AB包目录 (Assets/Download): 存放打包资源，设置AssetBundle标签",
            MessageType.Info
        );
    ...
    {
        EditorGUILayout.LabelField("AB包目录结构", EditorStyles.boldLabel);
        ...
        // 控制选项
        EditorGUILayout.BeginHorizontal();
        generateDownloadFolders = EditorGUILayout.Toggle("生成AB包目录", generateDownloadFolders);
        if (GUILayout.Button("全选"))
        {
    ...
    private void GenerateSelectedFolders()
    {
        ...
        EditorUtility.DisplayDialog("完成",
            ...
            $"AB包目录: {downloadCreatedCount} 个文件夹", "确定");
...
- 控制单个AB包大小";
    ...
    private void CleanEmptyFolders()
    {
            ...
            "确定要清理两个目录下的所有空文件夹吗？", "是", "否"))
        {
            ...
            EditorUtility.DisplayDialog("完成",
                $"清理完成！\n美术目录: {artRemoved} 个空文件夹\nAB包目录: {downloadRemoved} 个空文件夹", "确定");
        }
    }
    ...
    private void SetAssetBundleLabels()
    {
        if (!Directory.Exists(DownloadRootPath))
        {
            EditorUtility.DisplayDialog("错误", "AB包目录不存在，请先生成目录结构", "确定");
            return;
        }

        // 这里可以添加自动设置AssetBundle标签的逻辑
        // 例如根据文件夹结构自动设置标签名

        EditorUtility.DisplayDialog("提示",
            "AssetBundle标签设置功能待实现\n" +
            "请手动在Inspector中为Resources/Download下的资源设置AB标签", "确定");
    }

    [MenuItem("Tools/AssetBundle/快速生成核心结构")]
