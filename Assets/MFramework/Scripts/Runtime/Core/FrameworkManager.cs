...
public class FrameworkManager
{
    ...
    public LoggerModule LoggerModule => GetModule<LoggerModule>();
    ...
    public void RegisterModule<T>(T module) where T : IGameModule
    {
        ...
        {
            LoggerModule.LogWarning($"模块 {type.Name} 已经注册过了", LogType.FrameNormal);
            return;
        }
        ...
        LoggerModule.Log($"模块 {module.GetType()} 注册成功", LogType.FrameCore);
    }
    ...
    public void Update()
    {
        ...
        foreach (var module in _updateModules)
        {
            ...
            {
                LoggerModule.LogError($"模块 {module.GetType().Name} 更新异常: {e}", LogType.FrameCore);
            }
        }
    }
