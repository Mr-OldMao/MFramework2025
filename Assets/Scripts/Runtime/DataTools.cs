...
namespace GameMain
{
    public class DataTools
    {
        ...
        private static readonly Dictionary<int, FB_prop_prop> dicPropProp = new Dictionary<int, FB_prop_prop>();
        ...
        {
            var bytesData = await GameEntry.Resource.LoadAssetAsync<TextAsset>(SystemConstantData.PATH_CONFIG_DATA_ROOT + "bullet_bullet");
        ...
        {
            var bytesData = await GameEntry.Resource.LoadAssetAsync<TextAsset>(SystemConstantData.PATH_CONFIG_DATA_ROOT + "level_level");
            ByteBuffer byteBuffer = new ByteBuffer(bytesData.bytes);
        ...
        public static async Task SetPropProp()
        {
            var bytesData = await GameEntry.Resource.LoadAssetAsync<TextAsset>(SystemConstantData.PATH_CONFIG_DATA_ROOT + "prop_prop");
            ByteBuffer byteBuffer = new ByteBuffer(bytesData.bytes);
            var datas = FB_prop_prop_Array.GetRootAsFB_prop_prop_Array(byteBuffer);
            for (int i = 0; i < datas.DatasLength; i++)
            {
                dicPropProp.Add(datas.Datas(i).Value.ID, datas.Datas(i).Value);
            }
        }
        public static async Task SetTankPlayer()
        {
            var bytesData = await GameEntry.Resource.LoadAssetAsync<TextAsset>(SystemConstantData.PATH_CONFIG_DATA_ROOT + "tank_player");
            ByteBuffer byteBuffer = new ByteBuffer(bytesData.bytes);
        ...
        {
            var bytesData = await GameEntry.Resource.LoadAssetAsync<TextAsset>(SystemConstantData.PATH_CONFIG_DATA_ROOT + "tank_enemy");
            ByteBuffer byteBuffer = new ByteBuffer(bytesData.bytes);
